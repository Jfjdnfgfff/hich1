...
products.forEach((p, idx) => {
  ...
  htmlStr += `
              <!-- Product ${idx + 1} -->
              <div class="product-item bg-white border flex flex-col group hover:shadow-lg transition" data-category="${p.category}">
                <div class="relative p-6 flex-1 flex justify-center items-center h-48 sm:h-56 bg-white">
                  <img src="${p.img}" alt="${p.title}" class="max-h-full object-contain mix-blend-multiply group-hover:scale-105 transition duration-300" />
                </div>
                ...
                <div class="p-3 sm:p-4 flex flex-col flex-1 border-t">
                  <h4 class="text-[11px] sm:text-xs font-bold mb-2 leading-snug line-clamp-2 uppercase">
                    ${p.title}
                  </h4>
                  <div class="text-gray-300 text-[10px] sm:text-xs mb-3 flex items-center gap-0.5">
                    ${starsHtml}
                  ...
                  <div class="mt-auto flex gap-2 h-[48px]">
                    <input type="number" value="1" min="1" class="border border-gray-200 w-[60px] text-center text-sm font-semibold focus:outline-none focus:border-black rounded-sm bg-white" />
                    <button class="bg-black hover:bg-gray-900 transition text-white text-[10px] sm:text-xs font-bold px-2 sm:px-4 flex-1 uppercase tracking-widest rounded-sm flex items-center justify-center">
                      AJOUTER AU PANIER
                    </button>
                  </div>
                </div>
...
const productsStart = htmlFile.indexOf('<!-- Product 1 -->');
const productsEnd = htmlFile.indexOf('</div>\n          </div>\n        </section>\n      </div>\n\n      <!-- Packs View -->');

if (productsStart !== -1 && productsEnd !== -1) {
    htmlFile = htmlFile.substring(0, productsStart) + htmlStr + '            ' + htmlFile.substring(productsEnd);
    fs.writeFileSync('index.html', htmlFile);
    console.log('Products reverted successfully!');
} else {
    console.log('Could not find products boundaries');
}
